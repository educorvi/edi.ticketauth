...
class Newticket(BrowserView):
    def __call__(self):
        ticketlogin = ploneapi.portal.get_registry_record(
            "edi.ticketauth.configpanel.IEdiTicketSettings.ticketlogin"
        )
        ticketpassword = ploneapi.portal.get_registry_record(
            "edi.ticketauth.configpanel.IEdiTicketSettings.ticketpassword"
        )
        self.login = {"login": ticketlogin, "password": ticketpassword}
        self.formtitle = ploneapi.portal.get_registry_record(
            "edi.ticketauth.configpanel.IEdiTicketSettings.formtitle"
        )
        self.formhelp = ploneapi.portal.get_registry_record(
            "edi.ticketauth.configpanel.IEdiTicketSettings.formhelp"
        )
    ...
    def create_ticket(self, email):
        ...
        if resultdata["status"] == "success":
            ...
            ploneapi.portal.show_message(
                message=statusmessage, request=self.request, type="info"
            )
        else:
            ploneapi.portal.show_message(
                message=resultdata["message"], request=self.request, type="error"
            )
        return
