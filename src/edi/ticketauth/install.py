...
manage_add_editicketauth_form = PageTemplateFile(
    "browser/add_plugin", globals(), __name__="manage_add_editicketauth_form"
)
...
def manage_add_editicketauth_helper(dispatcher, id, title=None, REQUEST=None):
    ...
    if REQUEST is not None:
        REQUEST["RESPONSE"].redirect(
            "%s/manage_workspace"
            "?manage_tabs_message="
            "inwimandantHelper+added." % dispatcher.absolute_url()
        )
...
def register_editicketauth_plugin_class(context):
    context.registerClass(
        plugin.EdiTicketAuth,
        permission=manage_users,
        constructors=(manage_add_editicketauth_form, manage_add_editicketauth_helper),
        visibility=None,
